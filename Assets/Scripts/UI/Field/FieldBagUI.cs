...
public class FieldBagUI : MonoBehaviour
{
    ...
    public void Init(List<GameObject> playerPokemons, List<Item> bag, PlayerMovement playerMovement)
    {
    ...
    private void OnItemButtonClick(int index)
    {
        ...
        if (bag[index].type == ItemType.MONSTERBALL) return;

        fieldTextContent.text = "어떤 포켓몬에게 사용할까?";
    ...
    private void OnPokemonButtonClick(int index)
    {
        ...
        if (!item.CheckItemUseCondition(playerPokemons[index].GetComponent<Pokemon>()))
        {
            ...
            fieldTextContent.text = "사용할 수 없다!";
            fieldText.SetActive(true);
            StartCoroutine(BattleTextOff());
            return;
        }
    ...
    private IEnumerator HPUpdate(Pokemon pokemon, GameObject pokemonBtn)
    {
        ...
        fieldTextContent.text = pokemon.Name + "(은)는 건강해졌다!";
        fieldText.SetActive(true);
        yield return new WaitForSeconds(0.5f);
        yield return new WaitUntil(() => Input.anyKeyDown);
        fieldText.SetActive(false);
