...
public class PokemonChangeUI : MonoBehaviour
{
    private Type type;
    ...
    public void Init(List<GameObject> playerPokemons)
    {
    ...
    /// <summary>
    /// Should be called when change button clicked
    /// </summary>
    ...
    {
        if(this.selectedPokemon == selectedPokemon)
        {
            if (!playerPokemons[selectedPokemon].GetComponent<Pokemon>().Defeat) 
            {
                // Selecting Next Poke
                if (isPokemonDefeted)
                {
                    ...
                    BattleManager.Instance.SelectNextPokemon(selectedPokemon);
                    GetComponent<Canvas>().enabled = false;
                    isPokemonDefeted = false;
                    return;
                }

                // Selecting Leaving Poke
                else if (isPokemonCatched)
                {
                    for (int i = 0; i < playerPokemons.Count; i++)
                    {
                    ...
                    BattleManager.Instance.SetLeavingPokemon(selectedPokemon);
                    GetComponent<Canvas>().enabled = false;
                ...
                // Selecting Poke
                else if (BattleProcessor.Instance.GetPlayerPokemon().gameObject != playerPokemons[selectedPokemon])
                {
                    for (int i = 0; i < playerPokemons.Count; i++)
                    {
                        if (playerPokemons[i] == BattleProcessor.Instance.GetPlayerPokemon().gameObject) continue;
                    ...
                    BattleManager.Instance.SetPlayerCommand(new Command() { type = Command.TYPE.POKEMON, number = selectedPokemon });
                    GetComponent<Canvas>().enabled = false;
                    return;
                }
            }
    ...
    public void ReturnButton()
    {
        ...
        if (isPokemonCatched)
        {
            BattleManager.Instance.SetLeavingPokemon(-1);
            GetComponent<Canvas>().enabled = false;
            isPokemonCatched = false;
            return;
        }
